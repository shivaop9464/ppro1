  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (!isRazorpayConfigured) {
        // For testing without Razorpay, create order directly
        try {
          const orderResult = await supabaseService.createOrder({
      ...
      // Create order in database before initiating Razorpay payment
      let orderId = null;
      try {
        const orderResult = await supabaseService.createOrder({
          planId: selectedPlan?.id,
