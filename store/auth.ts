...
export const useAuthStore = create<AuthState>()((set, get) => ({
  ...
  initialize: async () => {
    ...
    try {
      // If Clerk is configured, rely on Clerk for authentication state
      if (isClerkConfigured() && useUser) {
        // Clerk handles authentication state, so we don't need to initialize anything here
        // The Navigation component will handle displaying Clerk user info
        set({ 
          user: null, 
          isAuthenticated: false, 
          loading: false, 
  ...
  login: async (email: string, password: string) => {
    ...
    try {
      // If Clerk is configured, don't use fallback authentication
      if (isClerkConfigured() && useUser) {
        console.log('Clerk is configured, use Clerk login instead of fallback');
        set({ loading: false });
        return false;
      }
  ...
  signup: async (name: string, email: string, password: string) => {
    ...
    try {
      // If Clerk is configured, don't use fallback authentication
      if (isClerkConfigured() && useUser) {
        console.log('Clerk is configured, use Clerk signup instead of fallback');
        set({ loading: false });
        return false;
      }
  ...
  logout: async () => {
    ...
    try {
      // If Clerk is configured, don't handle logout here (Clerk handles it)
      if (isClerkConfigured() && useUser) {
        set({ 
          user: null, 
          isAuthenticated: false, 
...
if (typeof window !== 'undefined') {
  ...
  // Only initialize if Clerk is not configured
  if (!isClerkConfigured()) {
