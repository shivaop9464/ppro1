...
export class SupabaseService {
  ...
  async getAllToys() {
    // If using fallback auth, return empty array or mock data
    if (!this.isSupabaseConfigured()) {
  ...
  async isAdmin() {
    // First check if Supabase is configured and working
    if (this.isSupabaseConfigured()) {
      try {
        const result = await this.getCurrentUserProfile();
        return result?.data?.is_admin || false;
      } catch (error) {
        console.warn('Error checking admin status via Supabase, falling back to localStorage check:', error);
      }
    }
    
    // Fallback to checking localStorage for admin status
    if (typeof window !== 'undefined') {
      ...
      if (fallbackUser) {
        try {
          ...
          console.error('Error parsing fallback user for admin check:', error);
        }
      }
    }
